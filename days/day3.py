...
def find_batteries(lines, ons=2):
    ...
    for l in lines:
        ...
        for rem in range(ons - 1, -1, -1):
            if rem > 0:
                maxidx, maxv = max(enumerate(l[maxidx + 1:-rem], start=maxidx+1), key=operator.itemgetter(1))
            else:
                maxidx, maxv = max(enumerate(l[maxidx + 1:], start=maxidx+1), key=operator.itemgetter(1))
            res = res * 10 + int(maxv)
        sm += res
    return sm
